...
function PaymentForm({ summary, onBack, onComplete, agent, agentRefetch, selAddon, unselAddon }: PaymentPageProps) {
  ...
  const existing = parseFloat(subscriptionData[0]?.tier?.annualPrice) + parseFloat(subscriptionData[0]?.addons?.reduce((acc: any, addon: any) => acc + addon.unitPrice * addon.quantity, 0))
  const isUpgrade = summary?.totals?.subtotal > existing;
  useEffect(() => {
    if (!elements) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ STEP 2: Card payment only for new or upgrade
      ...
      if (isUpgrade) {
        await upgradeSubscription({
          id: subscriptionData[0]?.id, payload: {
      ...
      } else if (!isUpgrade && hasExistingSubscription) {
