...
export const formatAuditLog = (log: any) => {
  const performer = log.performedBy?.name ?? 'Unknown'
  const time = new Date(log.createdAt).toLocaleString()

  const messageMap: Record<string, Record<string, string>> = {
    Quote: {
      CREATE: `New quote ${log.newValue?.quoteNumber ?? ''} generated by ${performer}`,
      APPROVE: `Quote approved by ${performer}`,
      UPDATE: `Quote updated by ${performer}`,
      DELETE: `Quote deleted by ${performer}`,
    },
    Organization: {
      CREATE: `New customer signed up`,
      UPDATE: `Organization updated by ${performer}`,
      DELETE: `Organization removed by ${performer}`,
    },
    Pricing: {
      CREATE: `New pricing tier created by ${performer}`,
      UPDATE: `Pricing tier updated by ${performer}`,
      DELETE: `Pricing tier removed by ${performer}`,
    },
  }

  const message =
    messageMap[log.entityType]?.[log.action] ??
    `${log.entityType} ${log.action} by ${performer}`
  ...
  return {
    ...
    time,
    action: log.action,
    color: colorMap[log.action] ?? 'bg-gray-100 text-gray-600',
    amount: log.newValue?.totalAmount ?? null,
  }
}
        ...
        <div className="bg-white border border-[#044866]/10 rounded-xl p-5">
          ...
          <div className="space-y-3">
            {auditLoading ? (
              ...
              auditData?.slice(0, 5).map((log: any) => {
            ...
            {agentAnalytics?.leaderboard
              ...
              .map((item: any, idx: number) => (
                <div key={item.agent.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-[#044866]">{item.agent.name}</div>
                    <div className="text-xs text-gray-600">${(item.revenue / 1000).toFixed(0)}k sales</div>
                  </div>
